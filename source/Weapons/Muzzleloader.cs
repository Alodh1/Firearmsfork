...
public class MuzzleloaderClient : RangeWeaponClient
{
    ...
    protected virtual bool Load(ItemSlot slot, EntityPlayer player, ref int state, ActionEventData eventData, bool mainHand, AttackDirection direction)
    {
        ...
        player.WalkInventory(slot =>
        {
            ...
            if (
                ...
                FirearmsAmmoUtility.HasEnoughBulletItems(slot, Stats.BulletLoadedPerReload, Stats.BulletsLoadedPerBulletItem))
            {
                ammoSlot = slot;
                return false;
            }
        ...
        string loadAnimation = GetLoadingAnimation(slot, Stats.LoadAnimation);
    ...
    protected virtual bool Cocking(ItemSlot slot, EntityPlayer player, ref int state, ActionEventData eventData, bool mainHand, AttackDirection direction)
    {
        ...
        TpAnimationBehavior?.Play(mainHand, cockingAnimation, category: AnimationCategory(mainHand), animationSpeed: animationSpeed);
        SetState(MuzzleloaderState.Cocking, mainHand);

        RangedWeaponSystem.SendStatusChange(player, RangedWeaponStatus.StartLoading, mainHand);
        Api.World.RegisterCallback(_ => CockingCallback(mainHand, player), GetActionDelayMs(player, cockingAnimation, animationSpeed, 600));
    ...
    {
        if (!CheckState(mainHand, MuzzleloaderState.Shoot)) return;
    ...
    protected virtual void ApplyRecoil(ItemSlot slot, EntityPlayer player)
    {
    ...
    protected virtual float EstimateShotDamage(ItemSlot slot)
    {
        ...
        float weaponMultiplier = Math.Max(0.1f, Stats.BulletDamageMultiplier * stackStats.DamageMultiplier);

        // Do not multiply recoil by pellet/projectile count.
        // Shotguns/blunderbusses already fire many projectiles and otherwise hit the pitch clamp instantly.
    ...
    {
        if (Stats.WaddingUsedPerReload <= 0) return true;
...
public class MuzzleloaderServer : RangeWeaponServer
{
    ...
    public override bool Reload(IServerPlayer player, ItemSlot slot, ItemSlot? ammoSlot, ReloadPacket packet)
    {
        ...
        if ((powderNeeded > 0 && flask?.Itemstack?.Item == null) ||
            (Stats.WaddingUsedPerReload > 0 && wadding?.Itemstack?.Item == null && currentStage == MuzzleloaderLoadingStage.Loading))
        {
            return false;
        }
        ...
        if (ammoSlot != null)
        {
            ...
            if (
                ...
                FirearmsAmmoUtility.HasEnoughBulletItems(ammoSlot, Stats.BulletLoadedPerReload, Stats.BulletsLoadedPerBulletItem))
            {
        ...
        if (wadding != null && Stats.WaddingUsedPerReload > 0 && currentStage == MuzzleloaderLoadingStage.Loading)
        {
