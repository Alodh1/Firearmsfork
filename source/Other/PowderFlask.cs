...
public class PowderFlask : Item
{
    ...
    public override bool OnHeldInteractStep(float secondsPassed, ItemSlot slot, EntityAgent byEntity, BlockSelection blockSelection, EntitySelection entitySel)
    {
        ...
        AssetLocation sound = new(Stats.RefillSound);
        byEntity.Api.World.PlaySoundAt(sound, atPlayer: (byEntity as EntityPlayer).Player, dualCallByPlayer: (byEntity as EntityPlayer).Player, randomizePitch: false, range: 16, volume: 1f);
